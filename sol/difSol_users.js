  ...
  .then(userPosts => {
   userPosts.length
    ...
    : res
       .status(404)
       .json({
        message: 'Either this user does not exist or they have no posts',
       });
  })
  .catch(err => {
   res.status(500).json({ errorMessage: err });
 ...
 if (userChanges.name) {
  userDb
   ...
   .catch(err => {
    res
     .status(500)
     .json({ errorMessage: 'There was an error updating the user.' });
   });
 } else {
 ...
 userDb
  ...
  .then(user => {
   if (user) {
    ...
    res
     .status(400)
     .json({ message: 'Given user id is invalid. User does not exist.' });
   }
  })
  .catch(err => {
