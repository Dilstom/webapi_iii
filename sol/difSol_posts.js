...
router.post('/', (req, res) => {
 ...
 if (newPost.text && newPost['user_id']) {
  ...
  res
   .status(400)
   .json({ message: 'Please include post text and the user ID.' });
 }
});
...
router.put('/:id', (req, res) => {
 ...
 if (postChanges.text || postChanges['user_id']) {
  ...
  res
   ...
   .json({
    message:
     'Please include either text or user_id information for the post changes.',
   });
 }
});
   ...
   postDb
    ...
    .then(countOfDeleted => {
     if (countOfDeleted) {
      ...
      res
       .status(400)
       .json({ message: 'Invalid Id. This post does not exist.' });
     }
    })
    .catch(err => {
